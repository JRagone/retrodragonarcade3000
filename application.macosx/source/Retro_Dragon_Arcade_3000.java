...
public class Retro_Dragon_Arcade_3000 extends PApplet {
...
//breakout
  public void bloc(int x,int y, int n){
     rectMode(CENTER);
     if(bloc[n]==true){
       rect(x,y,200,30);
  ...
  public void platform(int platformx, int platformy){
    fill(255,255,255);
    rectMode(CENTER);
...
public void draw(){
  ...
  //mario
    if(gameScreen==0){
      ...
      textX = width*.52f;
      titleY = height*.2f;
      duckHomeY = titleY + height*.08f;
      breakoutHomeY = titleY + height*.13f;
      marioHomeY = titleY + height*.18f;
      fill(255);
      textFont(homeFont);
      textAlign(CENTER);
      textSize(15);
      ...
      if(mouseY<=duckHomeY+5 && mouseY>=duckHomeY-10 && mouseX>width*.35f && mouseX<width*.65f){
        fill(105, 180, 35);
        text("Duck Hunt", textX, duckHomeY);
        fill(255);
        if(mousePressed){
      ...
      if(mouseY<=breakoutHomeY+5 && mouseY>=breakoutHomeY-10 && mouseX>width*.35f && mouseX<width*.65f){
        fill(105, 180, 35);
        text("Breakout", textX, breakoutHomeY);
        fill(255);
        if(mousePressed){
      ...
      if(mouseY<=marioHomeY+5 && mouseY>=marioHomeY-10 && mouseX>width*.35f && mouseX<width*.65f){
        fill(105, 180, 35);
        text("Mario", textX, marioHomeY);
        fill(255);
        if(mousePressed){
    ...
    if(gameScreen==3){
      ...
      //Main draw
        ...
        //Draw sprites
        image(marioBackground, mX/3 - width*.5f, -height, width*2, height*2);
        image(marioBackground, width*2+(mX/3 - width*.5f), -height, width*2, height*2);
        tint(255);
        goal.creation();
        Ground.creation();
        Coins.creation();
      ...
      //If lost a life
        ...
        image(angelMario, width*.14f, height*.1f);
        text(lifeCount, width*.3f, height*.165f);
...
//mario
  ...
  class goalPost {
    ...
    public void creation(){
      goalPostX = mX + width*2.5f;
      goalPost.resize(0, 400);
      image(goalPost, goalPostX, height - height*.12f - goalPost.height);
    }
  ...
  class pointerArrow{
    ...
    public void creation(){
      ...
      if(pointerUp){
       ...
       image(pointer, width*.3f, pointerY);
    }
  ...
  class player extends gravityObject {
    ...
    public void movement(){
      ...
      if(mY<groundHeight(oppositeMX)-mario.height*1.1f && mario!=cMario && mYV<0){
        mario = jMario;
        jumpResize();
      }
      if(mY<groundHeight(oppositeMX)-mario.height*1.1f && mario!=cMario && mYV>=0){
    ...
    public void fallResize(){
      mario.resize(0, 65);
    }
    
    public void jumpResize(){
      mario.resize(0, 70);
    }
    
    public void lookUpResize(){
      mario.resize(0,68);
    }
  ...
  class gravityObject {
    ...
    public float[] gravity(float x, float y, float v, PImage sprite){
 ...
 public PImage alternateImages(int frequency, PImage baseImage, PImage firstImage, boolean bounce, PImage secondImage){
    if(alternateStart){
      alternate = 0;
  ...
  class HUD {
    ...
    public void creation(){
      ...
      image(marioHUDHeader, width*.08f, height *.05f);
      xHUD.resize(0, 24);
      image(xHUD, width*.11f, height*.09f);
      text(lifeCount, width*.2f, height*.118f);
      
      //Timer
      timeHUD.resize (0,24);
      image(timeHUD, width*.65f, height*.055f);
      fill(247, 215, 121);
      text(displayTimer, width*.705f, height*.12f);
      fill(255);
      if(displayTimer>0){
        timer-=1/frameRate;
        displayTimer = (int) Math.ceil(timer);
      ...
      //Coins
      coinHUD.resize(0,24);
      image(coinHUD, width*.83f, height*.055f);
      text(coinCollectCount, width*.95f, height*.083f);
  ...
  class ground {
    ...
    public void creation(){
      ...
      i = 0;
      restartCreation = mX+width*1.5f+gap;
      for(level=1; level<=3; level++){
        for(gX = mX - width*.5f; gX<=mX+haltCreation; gX+=width/15){
          gY = height - level*(height*.04f);
          if(level==3){
            groundArray[i] = gY;
            i++;
            if(gX+width/15>mX+haltCreation){
              image(groundRCorner, gX, gY, width/15, height*.04f);
            }
            else{
              image(ground, gX, gY, width/15, height*.04f);
            }
          }
          else{
            if(gX+width/15>mX+haltCreation){
              image(belowGroundSide, gX, gY, width/15, height*.04f);
            }
            else{
              image(belowGround, gX, gY, width/15, height*.04f);
            }
          }
        }
        for(gX = restartCreation; gX<= mX+width*5; gX+=width/15){
          ...
          if(level==3){
            ...
            if(gX==restartCreation){
              pushMatrix();
              scale(-1, 1);
              image(groundRCorner, -gX - width/15, gY, width/15, height*.04f);
              popMatrix();
            }
            else{
              image(ground, gX, gY, width/15, height*.04f);
            ...
            if(gX==restartCreation){
              pushMatrix();
              scale(-1, 1);
              image(belowGroundSide, -gX - width/15, gY, width/15, height*.04f);
              popMatrix();
            }
            else{
              image(belowGround, gX, gY, width/15, height*.04f);
  ...
  public float groundHeight(float x){
    for(int i = 0; i<groundArray.length; i++){
      if(!wentLeft){
        if(x<=i*(width/15)+(width/15)/1.15f && x>=i*(width/15) - (width/15)/2){
...
//duckhunt
  public void mousePressed(){
    if(gameScreen==1){
      ...
      if(sequence==0&&mouseX<450&&mouseX>300&&mouseY>375&&mouseY<375+30){
        ...
        //makes ducks
        for(int i=0; i<duckHunter.length;i++){
        duckHunter[i]= new duck(random(0,width-40),height,5,random(2,5),(Math.random()<.5f),false);
      }
        sequence = 1;
      }
      //almost there
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Retro_Dragon_Arcade_3000" };
    if (passedArgs != null) {
